...
class CSVDataLoader:
    ...
    def row_processer(self, row):
        try:
            ...
            context_mask = torch.tensor(json.loads(row[2]))
            out_of_context_mask = torch.where((context_mask == 0))
            incontext_comment_mask = torch.tensor(list(set(torch.where((context_mask == 2))[0].tolist()) - set(label)))
        except:
            print("Error========================================57")
            return None
        ...
        NL_tokens[out_of_context_mask] = -1
        if (incontext_comment_mask.shape[0] != 0):
            NL_tokens[incontext_comment_mask] = 2
...
def save_data():
    ...
    for pretrain_type in pretrain_types:
        ...
        for batch_iter, batch in enumerate(tqdm(data_loaded, total=10266)):
