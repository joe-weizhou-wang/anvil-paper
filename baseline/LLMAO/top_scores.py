        ...
        for proj_idx in range(len(split_probs_proj)):
            ...
            for bug_idx in range(len(split_probs_bugs)):
                ...
                if correct_preds > 0:
                    hit_counter += 1
                total_counter += 1
                slice_length += len(prob_bug) / max(1, sum(label_bug))
        label_bug_counter = round(label_bug_counter / window_split)
        

        if n_top == 5:
            # print(total_counter)
            # print(label_bug_counter)
            # print(slice_length / total_counter)
            
            top_5 = hit_counter
        elif n_top == 3:
            top_3 = hit_counter
        elif n_top == 1:
            avg_slice_length += slice_length / total_counter
            top_1 = hit_counter
...
def results(log_path, data_name, codegen_size):
    ...
    if total_bugs:
        ...
        print(f"Average slice length: {avg_slice_length / 10 / 1.9}")
...
if __name__ == "__main__":
    ...
    data_name = "devign_new_all"

    current_path = os.getcwd()
    log_path = f"{current_path}/" + log_path
